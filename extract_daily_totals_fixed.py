...
def extract_nse_totals(filepath, date):
    ...
    try:
        ...
        with zipfile.ZipFile(filepath, 'r') as z:
            # Find the CSV file (might be nested in directory)
            csv_files = [f for f in z.namelist() if f.endswith('.csv')]
            if not csv_files:
                print(f"No CSV found in {filepath}")
                return None
            
            csv_name = csv_files[0]  # Take the first CSV file found
        ...
        for line in lines:
            ...
            if in_data and line.strip() and ',' in line:
                # Skip lines that are not data (like totals)
                if not line.startswith(',') and not 'Total' in line:
                    totals['securities_count'] += 1
        
        # Extract summary values - search in first 20 lines
        ...
        if totals['end_outstanding'] is None and totals['beginning_outstanding'] is None:
            print(f"Warning: No valid totals found in {filepath}")
...
def main():
    ...
    print("EXTRACTING DAILY TOTALS FROM MTF FILES (FIXED VERSION)")
    print("=" * 80)
    ...
    if os.path.exists(nse_dir):
        ...
        print(f"  NSE files with blank totals: {blank_count}")
    ...
    output_file = "mtf_daily_totals_fixed.csv"
    ...
    json_output = "mtf_daily_totals_fixed.json"
    ...
    if not blank_nse.empty:
        print(f"\nWARNING: {len(blank_nse)} NSE records have blank end_outstanding values")
        print("Sample of blank records:")
        print(blank_nse.head(10)[['date', 'securities_count']].to_string(index=False))
